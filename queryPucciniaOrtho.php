...
$query = 'SELECT * FROM orthologs WHERE species="'.$species.'"';
...
while ($lineQueryResult = mysqli_fetch_array($result, MYSQLI_ASSOC)){
	...
	$group = $lineQueryResult['orthoGroup'];
	$tc = $lineQueryResult['tcariesOrtho'];
	$description = $lineQueryResult['tlaevisOrtho'];

	$line = $group."\t". $tc."\t". $description. "\t";
	print_r($line);
	print_r('\n');
